...
import com.example.weatherforecast.ui.theme.composables.CardCylinder
import com.example.weatherforecast.ui.theme.composables.CardRectangle
import com.example.weatherforecast.ui.theme.composables.CardRectangleMultipleInformation
...
fun HomeScreen(
    ...
    Column(
        ...
        if (location != null && currentWeatherResponse != null) {
            CurrentWeatherDetails(location, currentWeatherResponse)
            HourlyDetails()
            Spacer(modifier = Modifier.Companion.height(Dimensions.paddingModerate))
            DailyDetails()
            Spacer(modifier = Modifier.Companion.height(Dimensions.paddingModerate))
            PreviousFiveDaysDetails()
        } else {
            // Will show a dialog
...
@Composable
fun CurrentWeatherDetails(location: Location?, currentWeatherResponse: CurrentWeatherResponse?) {
...
fun DailyDetails() {
    Column {
        ...
        {
            CardRectangleMultipleInformation()
        }
    }
}
...
@Composable
fun PreviousFiveDaysDetails() {
    Column {
        TextLarge(stringResource(R.string.past_5_days))
        Spacer(modifier = Modifier.Companion.height(Dimensions.paddingModerate))
        Column(
            verticalArrangement = Arrangement.spacedBy(Dimensions.paddingSmall),
            modifier = Modifier.padding(bottom = Dimensions.paddingModerate)
